...
fn subscribe_pkt(local_id: u16, tx_ch: &str, tx_host: &str) -> Vec<u8> {
    ...
    body.push(0);
    body[4..6].copy_from_slice(&name_off.to_be_bytes());
    body[6..8].copy_from_slice(&host_off.to_be_bytes());
...
impl FakeTx {
    async fn start() -> Self {
        ...
        let join = tokio::spawn(async move {
            fake_tx_loop(fc, stop2, ka).await;
        });
        Self {
            flows_port,
            stop,
...
async fn fake_tx_loop(fc: UdpSocket, stop: Arc<AtomicBool>, keepalives: Arc<AtomicU32>) {
            ...
            _ = tick.tick() => {
                if sending
                    && let Some(d) = dest {
                        ...
                        let frames = 16u64;
                        let mut samples = vec![0i32; frames as usize];
                        for (i, s) in samples.iter_mut().enumerate() {
                            *s = ((idx as i32).wrapping_add(i as i32)) << 16;
                        }
