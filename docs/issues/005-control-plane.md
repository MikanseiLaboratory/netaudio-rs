...
Phase 1。プロトコル（[#3](https://github.com/MikanseiLaboratory/netaudio-rs/issues/3)）とソケット（[#5](https://github.com/MikanseiLaboratory/netaudio-rs/issues/5)）を使い、プロセスを Dante デバイスとして広告する。

親: [#2](https://github.com/MikanseiLaboratory/netaudio-rs/issues/2)。Inferno `dev` の `device_server/{mdns_server,arc_server,cmc_server,info_mcast_server,flows_control_server}.rs`。
...
- CMC でデバイスとして認識される
- info multicast の heartbeat 骨格（overlay 準備前でも広告は出す）
